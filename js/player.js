...
let PlayerMoves = {
    calcAttack: function(){
        ...
        if (getPlayerSpeed >= getEnemySpeed){
            ...
            enemy.health= enemy.health - totalDamage; //update enemy health to equal current value minus totalDamage calculated in PlayerAttack function
            ...
            alert(`Enemy hit you ${enemyAttackValues[1]} times for ${enemyAttackValues[0]} damage`)
            if (player.health <= 0) {
                alert ("You LOSE!!! Refresh Browser to Play again")
                getPlayerHealth.innerHTML = 'Health: 0'; //set play
                getEnemyHealth.innerHTML = `Health: ${enemy.health}`;
            } else {
                getPlayerHealth.innerHTML = `Health: ${player.health}` 
            }
